...
namespace webots_ros2_driver
{
    ...
    {
        std::string moduleName = type.substr(0, type.find_last_of("."));
        std::string className = type.substr(type.find_last_of(".") + 1);
        ...
        PyObject *pyClass = PyDict_GetItemString(pyClass, className.c_str());
        if (!pyClass)
