...
"""Test the `webots_ros2_epuck` package on the rats life scenario."""
...
def generate_test_description():
    ...
    epuck_rats_webots = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory('webots_ros2_epuck'), 'launch', 'rats_life_launch.py')
        )
    )
...
class TestEpuck(TestWebots):
    ...
    def testMap(self):
        ...
        self.wait_for_messages(self.__node, OccupancyGrid, '/local_costmap/costmap', condition=on_cost_map_message_received)
